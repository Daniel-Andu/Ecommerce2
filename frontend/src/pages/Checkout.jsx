...
export default function Checkout() {
  ...
  // ===== FIXED: handlePlaceOrder with proper Chapa redirect =====
    ...
    try {
      ...
      if (paymentMethod === 'chapa') {
        ...
        try {
          ...
          const paymentResponse = await api.payment.initialize({
            order_id: order.id
          });
          
          console.log('Payment response:', paymentResponse);

          // Check if we have a checkout URL to redirect to Chapa
          if (paymentResponse.checkout_url) {
            // REDIRECT TO CHAPA PAYMENT PAGE
            window.location.href = paymentResponse.checkout_url;
          } else if (paymentResponse.data && paymentResponse.data.checkout_url) {
            window.location.href = paymentResponse.data.checkout_url;
          } else if (paymentResponse.url) {
            window.location.href = paymentResponse.url;
          } else {
            // If no checkout URL (demo mode or error), go to confirmation
            console.log('No checkout URL, going to confirmation');
            toast.success('Order placed successfully!');
            navigate(`/order-confirmation/${order.id}`);
          }
        } catch (paymentError) {
          console.error('Payment error:', paymentError);
          toast.error('Payment initialization failed. Order created - you can try again later.');
          // Still go to confirmation since order was created
          navigate(`/order-confirmation/${order.id}`);
        }
      } else {
            ...
            {addresses.length > 0 && !showNewAddressForm ? (
                ...
                {addresses.map(addr => (
                  <label key={addr.id} className={`address-card ${selectedAddress === addr.id ? 'selected' : ''}`}>
                    ...
                    <div className="address-details">
                      <div className="address-name">{addr.full_name || 'Shipping Address'}</div>
                      <div className="address-text">{addr.address_line1}{addr.address_line2 && `, ${addr.address_line2}`}</div>
                      <div className="address-text">{addr.city}, {addr.state} {addr.postal_code}</div>
              ...
              <div className="address-form-container">
                <div className="form-header">
                  ...
                  {addresses.length > 0 && (
                    <button type="button" onClick={handleCancelAddressForm} className="close-form-btn">×</button>
                  )}
                </div>
                <form onSubmit={handleAddAddress} className="address-form">
                  <div className="form-row">
                    <div className="form-group full-width">
                      ...
                      <input type="text" name="full_name" value={newAddress.full_name} onChange={handleNewAddressChange} placeholder="John Doe" className={formErrors.full_name ? 'error' : ''} />
                      {formErrors.full_name && <span className="error-message">{formErrors.full_name}</span>}
                    </div>
                  </div>

                  <input type="hidden" name="label" value={newAddress.label} />

                  <div className="form-row">
                    <div className="form-group full-width">
                      <label>Address Line 1 <span className="required">*</span></label>
                      <input type="text" name="address_line1" value={newAddress.address_line1} onChange={handleNewAddressChange} placeholder="Street address" className={formErrors.address_line1 ? 'error' : ''} />
                  ...
                  <div className="form-row">
                    <div className="form-group full-width">
                      ...
                      <input type="text" name="address_line2" value={newAddress.address_line2} onChange={handleNewAddressChange} placeholder="Apartment, suite, etc." />
                    </div>
                  </div>
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <input type="text" name="city" value={newAddress.city} onChange={handleNewAddressChange} placeholder="City" className={formErrors.city ? 'error' : ''} />
                      {formErrors.city && <span className="error-message">{formErrors.city}</span>}
                    </div>
                    
                    <div className="form-group">
                      <label>State/Region</label>
                      <input type="text" name="state" value={newAddress.state} onChange={handleNewAddressChange} placeholder="State or region" />
                    </div>
                  </div>
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <input type="text" name="postal_code" value={newAddress.postal_code} onChange={handleNewAddressChange} placeholder="Postal code" />
                    </div>
                    
                    <div className="form-group">
                      <label>Country</label>
                      <select name="country" value={newAddress.country} onChange={handleNewAddressChange}>
                  ...
                  <div className="form-row">
                    <div className="form-group full-width">
                      ...
                      <input type="tel" name="phone" value={newAddress.phone} onChange={handleNewAddressChange} placeholder="+251 912 345 678" className={formErrors.phone ? 'error' : ''} />
                  ...
                  <div className="form-actions">
                    ...
                    {addresses.length > 0 && (
                      <button type="button" onClick={handleCancelAddressForm} className="btn-cancel">Cancel</button>
                    )}
                  </div>
                </form>
              </div>
          ...
          <div className="checkout-section">
            ...
            <div className="payment-methods">
              <label className={`payment-option ${paymentMethod === 'chapa' ? 'selected' : ''}`}>
                <input type="radio" name="payment" value="chapa" checked={paymentMethod === 'chapa'} onChange={() => setPaymentMethod('chapa')} />
                <div className="payment-details">
                  <span className="payment-name">Chapa Payment</span>
                  <span className="payment-description">Pay with Telebirr, CBE, Amole, or Credit Card</span>
                </div>
              </label>
