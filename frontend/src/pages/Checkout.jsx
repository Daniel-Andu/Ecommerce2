...
export default function Checkout() {
    ...
    const loadData = async () => {
      try {
        ...
        if (cart.items && cart.items.length > 0) {
          checkOutOfStockItems(cart.items);
        }
  ...
  // Check for out of stock items
  const checkOutOfStockItems = async (items) => {
    try {
      // Get all products to check stock
      const productsList = await products.list({ limit: 100 });
      ...
      if (outOfStock.length > 0) {
        toast.error(`${outOfStock.length} item(s) in your cart are out of stock`);
      }
    } catch (error) {
  ...
  const removeOutOfStockItems = async () => {
    ...
    const loadingToast = toast.loading('Removing out of stock items...');
    ...
    try {
      ...
      setCartData({ 
        items: cart.items || [], 
        subtotal: parseFloat(cart.subtotal || 0), 
        sessionId: cart.sessionId || localStorage.getItem('cartSession')
      });
  ...
  // 🔥 FIXED FORCE FIX CART FUNCTION - Only adds in-stock products
  const forceFixCart = async () => {
    setFixingCart(true);
    const fixToast = toast.loading('🔧 Fixing your cart...');
    ...
    try {
      ...
      // Step 3: Get all products and check stock
      const productsList = await products.list({ limit: 100 });
      const allProducts = productsList.products || [];
      
      // Step 4: Define products to add (only those with stock > 0)
      const productsToTry = [
        { id: 1, name: 'Samsung Galaxy S23', qty: 1 },
        { id: 2, name: 'Closes', qty: 2 },
        { id: 3, name: 'djhhfvh', qty: 1 },
      ...
      for (const product of productsToTry) {
        ...
        if (!productInfo) {
          console.log(`⚠️ Product ${product.name} not found in database`);
          skippedCount++;
          continue;
        }
        
        if (productInfo.stock_quantity === 0) {
          console.log(`⚠️ ${product.name} is out of stock (0 available)`);
          skippedCount++;
          continue;
        }
        ...
        try {
          const response = await fetch('/api/cart/items', {
            ...
            body: JSON.stringify({
              product_id: product.id,
              quantity: Math.min(product.qty, productInfo.stock_quantity) // Don't exceed available stock
            })
          });
          
          if (response.ok) {
            addedCount++;
            console.log(`✅ Added ${product.name} (stock: ${productInfo.stock_quantity})`);
          } else {
            skippedCount++;
            console.log(`❌ Failed to add ${product.name}`);
          }
      ...
      if (addedCount > 0) {
        toast.success(`✅ Added ${addedCount} products (${skippedCount} skipped - out of stock). Refreshing...`);
        setTimeout(() => window.location.reload(), 1500);
      } else {
        toast.error('❌ No products available to add. All may be out of stock.');
        setFixingCart(false);
      }
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      if (err.message && err.message.includes('Insufficient stock')) {
        ...
        // Reload cart to get updated stock info
        const cart = await cartApi.get();
        setCartData({ 
          items: cart.items || [], 
          subtotal: parseFloat(cart.subtotal || 0), 
      ...
      <div className="container">
        ...
        {outOfStockItems.length > 0 && (
          <div className="out-of-stock-warning" style={{
            ...
            <h3 style={{ margin: '0 0 10px 0' }}>⚠️ Out of Stock Items</h3>
            <p>The following items are out of stock and must be removed:</p>
            <ul>
        ...
        <div style={{ marginBottom: '20px', textAlign: 'center' }}>
          ...
          >
            {fixingCart ? '⏳ FIXING...' : '🔄 FIX CART (Auto-remove out of stock)'}
          </button>
          <p style={{ color: '#6B7280', marginTop: '10px', fontSize: '0.9rem' }}>
            This will clear your cart and only add products that are in stock
          </p>
        </div>
        ...
        <div className="checkout-layout">
          ...
          <aside className="order-summary">
            ...
            ) : cartData.items && cartData.items.length > 0 && outOfStockItems.length === 0 ? (
              <button 
                onClick={handlePlaceOrder}
                className="btn-place-order"
                disabled={placing}
