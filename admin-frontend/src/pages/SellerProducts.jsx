...
import { seller, categories } from '../services/api';  // Fixed import path
  ...
  const getProductImageUrl = (product) => {
    ...
    if (product.images && product.images.length > 0) {
      ...
      if (typeof image === 'string') {
        return getImageUrl(image, productPlaceholder);
      } else if (image && image.url) {
        return getImageUrl(image.url, productPlaceholder);
      }
    }
    
    if (product.image_url) {
      return getImageUrl(product.image_url, productPlaceholder);
    }
        ...
        <form onSubmit={handleSubmit} className="product-form">
          ...
          {formData.image_url && (
            <div className="image-preview">
              <img 
                src={getImageUrl(formData.image_url, productPlaceholder)} 
                alt="Preview" 
        ...
        <div className="products-table">
          ...
          {products.map(product => (
            <div key={product.id} className="table-row">
              ...
              <span>ETB{Number(product.sale_price || product.base_price).toFixed(2)}</span>
