...
// ==================== DASHBOARD STATS - NO GROUP BY VERSION ====================
router.get('/dashboard', async (req, res) => {
  try {
    console.log('📊 Fetching dashboard data - NO GROUP BY');
    
    // First, disable sql_mode for this session
    ...
    const [rawDailyOrders] = await pool.query(`
      SELECT 
    ...
    rawDailyOrders.forEach(order => {
      const day = order.day_name;
      if (dailyRevenueMap.has(day)) {
        const existing = dailyRevenueMap.get(day);
        existing.revenue += parseFloat(order.revenue || 0);
        existing.orders += 1;
      } else {
    ...
    const dailyRevenue = dayNames.map(day => {
      return dailyRevenueMap.get(day) || { name: day, revenue: 0, orders: 0 };
    });
    
    console.log(`📊 Daily revenue: ${dailyRevenue.filter(d => d.revenue > 0).length} days with data`);

    // ===== MONTHLY REVENUE (Last 30 days) =====
    const [rawMonthlyOrders] = await pool.query(`
      SELECT 
        DATE_FORMAT(created_at, '%d %b') as date_name,
        total as revenue
      FROM orders 
    ...
    rawMonthlyOrders.forEach(order => {
      const date = order.date_name;
      if (monthlyRevenueMap.has(date)) {
        const existing = monthlyRevenueMap.get(date);
        existing.revenue += parseFloat(order.revenue || 0);
        existing.orders += 1;
      } else {
        monthlyRevenueMap.set(date, {
          name: date,
          revenue: parseFloat(order.revenue || 0),
          orders: 1
        });
    ...
    console.log(`📊 Monthly revenue: ${monthlyRevenue.length} days with data`);

    // ===== CATEGORY DISTRIBUTION =====
    const [categoryData] = await pool.query(`
      SELECT 
        c.name,
        (SELECT COUNT(*) FROM products p WHERE p.category_id = c.id) as value
      FROM categories c
    ...
    const [topProducts] = await pool.query(`
      SELECT 
        ...
        COALESCE((SELECT SUM(total_price) FROM order_items WHERE product_id = p.id), 0) as revenue
      FROM products p
    ...
    const responseData = {
      ...
      revenueChange: parseFloat(revenueChange) || 12.5
    };

    console.log('✅ FINAL RESPONSE DATA COUNTS:', {
      dailyRevenue: responseData.dailyRevenue.filter(d => d.revenue > 0).length,
...
router.post('/banners/upload', bannerUpload.single('image'), (req, res) => {
  try {
    if (!req.file) {
...
// // ==================== DASHBOARD STATS - ULTRA SIMPLE VERSION ====================
// router.get('/dashboard', async (req, res) => {
//   try {
//     console.log('📊 Fetching SIMPLE dashboard data');
    
//     // Get basic counts - these ALWAYS work
//     const [[users]] = await pool.query('SELECT COUNT(*) AS total FROM users');
//     const [[sellers]] = await pool.query('SELECT COUNT(*) AS total FROM sellers WHERE status = "approved"');
//     const [[pendingSellers]] = await pool.query('SELECT COUNT(*) AS total FROM sellers WHERE status = "pending"');
//     const [[products]] = await pool.query('SELECT COUNT(*) AS total FROM products');
...
//     // Get recent orders - simple query
//     const [recentOrders] = await pool.query(`
//       SELECT 
//         o.id,
//         o.order_number,
...
//     // Send response with basic data and empty arrays for graphs
//     const responseData = {
//       totalUsers: users.total || 0,
//       totalSellers: sellers.total || 0,
//       pendingSellers: pendingSellers.total || 0,
...
//       // Empty arrays for graphs - at least dashboard loads
//       dailyRevenue: [],
//       monthlyRevenue: [],
...
//   } catch (err) { 
//     console.error('❌ Admin dashboard error:', err);
//     // Even on error, send something so frontend doesn't crash
//     res.json({
//       totalUsers: 0,
//       totalSellers: 0,
//       pendingSellers: 0,
