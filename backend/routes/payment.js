...
router.post('/initialize', auth, async (req, res) => {
  try {
    ...
    // Check if amount exceeds Chapa limit (1,000,000 ETB)
    if (order.total > 1000000) {
      return res.status(400).json({
        success: false,
        message: `Order total ${order.total} ETB exceeds Chapa's maximum limit of 1,000,000 ETB. Please split your order into multiple payments or reduce cart total.`
    ...
    if (!CHAPA_SECRET_KEY || CHAPA_SECRET_KEY === 'your_chapa_secret_key_here') {
      ...
      // In demo mode, automatically mark as paid
      setTimeout(async () => {
        try {
          await pool.query(
            'UPDATE orders SET payment_status = ?, status = ? WHERE id = ?',
      ...
      // FIX: Return the correct URL structure with order ID in the path
      return res.json({
        success: true,
        demo: true,
        message: 'Demo mode - payment simulated',
    ...
    const orderNumber = order.order_number || `ORD-${order.id}`;
    const title = `Order ${orderNumber.substring(0, 10)}`;
    const description = `Payment for order ${orderNumber}`.replace(/[^a-zA-Z0-9\s\-_\.]/g, '');

    // FIX: IMPORTANT - Set return_url to include order ID in path, not as query param
    const return_url = `${FRONTEND_URL}/order-confirmation/${order.id}?status=success`;
    const callback_url = `${BACKEND_URL}/api/payment/webhook`;
